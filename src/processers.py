...
class ProcesserClass:
    ...
    def prepare_luh2_data(self):
        ...
        #shutil.copy(os.path.join(scriptsdir, f"grid_{self.grid}"), os.path.join(luhdir, sdir, self.region))
        ...
        if self.trans:
            ...
            cdo.sellonlatbox(reg, input=f"-selyear,{self.syear}/{self.eyear} -selvar,{vars_trans} {luhdir}/{sdir}/{tfile}.nc", output=f"{luhdir}/{sdir}/{self.region}/{ofile}")
            # cdo sellonlatbox,$reg -selyear,$syear/$eyear -selvar,$vars_trans $sdir/$tfile.nc $sdir/$self.region/$ofile
        print(1)
        if self.state:
            #cdo sellonlatbox,$reg -selyear,$syear/$eyear -selvar,$vars_state $sdir/$sfile.nc $sdir/$self.region/states_${syear}_${eyear}_$self.region.nc
            cdo.sellonlatbox(reg, input=f"{sdir}/{sfile}.nc", output=f"{sdir}/{self.region}/states_{self.syear}_{self.eyear}_{self.region}.nc", options=f"-selyear,{self.syear}/{self.eyear} -selvar,{vars_state}")
            #cdo $remap_com $sdir/$self.region/states_${syear}_${eyear}_$self.region.nc $sdir/$self.region/states_${syear}_${eyear}_$grid.nc
            if remap_com == "invertlat":
                cdo.invertlat(input=f"{sdir}/{self.region}/states_{self.syear}_{self.eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/states_{self.syear}_{self.eyear}_{self.grid}.nc")
            elif remap_com == "remapbil":
                cdo.remapbil(f"grid_{grid}", input=f"{sdir}/{self.region}/states_{syear}_{eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/states_{self.syear}_{self.eyear}_{self.grid}.nc")
            elif remap_com == "remapcon2":
                cdo.remapcon2(f"grid_{grid}", input=f"{sdir}/{self.region}/states_{syear}_{eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/states_{self.syear}_{self.eyear}_{self.grid}.nc")
        if self.addtree:
            #cdo sellonlatbox,$reg -selyear,$syear/$eyear -selvar,added_tree_cover $sdir/$afile.nc $sdir/$self.region/addtree_${syear}_${eyear}_$self.region.nc
            #cdo $remap_com $sdir/$self.region/addtree_${syear}_${eyear}_$self.region.nc $sdir/$self.region/$grid/addtree_${syear}_${eyear}_$grid.nc
            #cdo -f srv copy $sdir/$self.region/$grid/addtree_${syear}_${eyear}_$grid.nc $sdir/$self.region/$grid/addtree_${syear}_${eyear}_$grid.srv
            cdo.sellonlatbox(reg, input=f"-selyear,{self.syear}/{self.eyear} -selvar,added_tree_cover {sdir}/{afile}.nc", output=f"{sdir}/{self.region}/addtree_{self.syear}_{self.eyear}_{self.region}.nc")
            if remap_com == "invertlat":
                cdo.invertlat(input=f"{sdir}/{self.region}/addtree_{self.syear}_{self.eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/{self.grid}/addtree_{self.syear}_{self.eyear}_{self.grid}.nc")
            elif remap_com == "remapbil":
                cdo.remapbil(f"grid_{self.grid}", input=f"{sdir}/{self.region}/addtree_{self.syear}_{self.eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/{self.grid}/addtree_{self.syear}_{self.eyear}_{self.grid}.nc")
            elif remap_com == "remapcon2":
                cdo.remapcon2(f"grid_{self.grid}", input=f"{sdir}/{self.region}/addtree_{self.syear}_{self.eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/{self.grid}/addtree_{self.syear}_{self.eyear}_{self.grid}.nc")
            cdo.copy(input=f'{sdir}/{self.region}/{self.grid}/addtree_{self.syear}_{self.eyear}_{self.grid}.nc', output=f"{sdir}/{self.region}/{self.grid}/addtree_{self.syear}_{self.eyear}_{self.grid}.srv")

        # compute irragtion fraction 
        if self.irri:
            cdo.sellonlatbox(reg, input=f"-selyear,{self.syear}/{self.eyear} -selvar,{vars_irrig} {sdir}/{mfile}.nc", output=f"{sdir}/{self.region}/irri_vars_{self.syear}_{self.eyear}_{self.region}.nc")

            if self.scenario in ["historical", "historic_low", "historic_high"]:
                if remap_com == "invertlat":
                    cdo.invertlat(input=f"-chname,{IRR[0]},irrig_frac -selvar,{IRR[0]} {sdir}/{self.region}/irri_vars_{self.syear}_{self.eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/{grid}/irrigation_{syear}_{eyear}_{grid}.nc")
                elif remap_com == "remapbil":
                    cdo.remapbil(f"grid_{grid}", input=f"-chname,{IRR[0]},irrig_frac -selvar,{IRR[0]} {sdir}/{self.region}/irri_vars_{self.syear}_{self.eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/{self.grid}/irrigation_{syear}_{eyear}_{grid}.nc")
                elif remap_com == "remapcon2":
                    cdo.remapcon2(f"grid_{grid}", input=f"-chname,{IRR[0]},irrig_frac -selvar,{IRR[0]} {sdir}/{self.region}/irri_vars_{self.syear}_{self.eyear}_{self.region}.nc", output=f"{sdir}/{self.region}/{self.grid}/irrigation_{syear}_{eyear}_{grid}.nc")
                cdo.copy(options="-setmisstoc,-999", input=f'{sdir}/{self.region}/{self.grid}/irrigation_{self.syear}_{self.eyear}_{self.grid}.nc', output=f"{sdir}/{self.region}/{self.grid}/irrigation_{self.syear}_{self.eyear}_$grid.srv")
            else:
                # cdo setmisstoc,-999 -$remap_com -varssum -selvar,$vars_crops $sdir/$self.region/states_${syear}_${eyear}_$grid.nc $sdir/$self.region/sum_crop_frac.nc
                if remap_com in ["remapbil", "remapcon2"]:
                    cdo.setmisstoc(input=f"-999 -{remap_com}, grid_{self.grid} -varssum -selvar,{vars_crops} {sdir}/{self.region}/states_{self.syear}_{self.eyear}_{self.grid}.nc", output=f"{sdir}/{self.region}/sum_crop_frac.nc")
                else:
                    cdo.setmisstoc(input=f"-999 -{remap_com} -varssum -selvar,{vars_crops} {sdir}/{self.region}/states_{self.syear}_{self.eyear}_{self.grid}.nc", output=f"{sdir}/{self.region}/sum_crop_frac.nc")

                # Change variables names

                for n in range(5):
                    if remap_com in ["remapbil", "remapcon2"]:
                        cdo.mul(input=f"-{remap_com}, grid_{grid} -selvar,{IRR[n]} {sdir}/{self.region}/irri_vars_{self.syear}_{self.eyear}_{self.region}.nc -{remap_com}, grid_{self.grid} -selvar,{ICR[n]} {sdir}/{self.region}/states_{self.syear}_{self.eyear}_{self.grid}.nc", output=f"{sdir}/{self.region}/dummy.nc")
                    else:
                        cdo.mul(input=f"-{remap_com} -selvar,{IRR[n]} {sdir}/{self.region}/irri_vars_{self.syear}_{self.eyear}_{self.region}.nc -{remap_com} -selvar,{ICR[n]} {sdir}/{self.region}/states_{syear}_{eyear}_{grid}.nc", output=f"{sdir}/{self.region}/dummy.nc")
                    if n == 0:
                        cdo.chname(input=f"{IRR[0]},irrig_frac -selvar,{IRR[0]} {sdir}/{self.region}/dummy.nc", output=f"{sdir}/{self.region}/sum_irri_frac_{self.syear}_{self.eyear}_{self.grid}.nc")
                    else:
                        cdo.add(input=f"-chname,{IRR[n]},irrig_frac -selvar,{IRR[n]} {sdir}/{self.region}/dummy.nc {sdir}/{self.region}/sum_irri_frac_{self.syear}_{self.eyear}_{self.grid}.nc")   
                        shutil.move(f"{sdir}/{self.region}/dummy2.nc", f"{sdir}/{self.region}/sum_irri_frac_{self.syear}_{self.eyear}_{self.grid}.nc")
                    os.remove(f"{sdir}/{self.region}/dummy.nc")
                cdo.div(input=f"{sdir}/{self.region}/sum_irri_frac_{self.syear}_{self.eyear}_{self.grid}.nc {sdir}/{self.region}/sum_crop_frac.nc", output=f"{sdir}/{self.region}/{self.grid}/irrigation_{self.syear}_{self.eyear}_{self.grid}.nc")
                cdo.copy(input=f"-setmisstoc,-999 {sdir}/{self.region}/{self.grid}/irrigation_{self.syear}_{self.eyear}_{self.grid}.nc", output=f"{sdir}/{self.region}/{self.grid}/irrigation_{self.syear}_{self.eyear}_{self.grid}.srv")
        if self.trans:
...
def fromto(varn, for_1, for_2, tfile, ext, cutting, outvar_condition=None):
    ...
    ifile=f"{tfile}_{syear}_{eyear}_{self.region}.nc"
    logfile=f"{tfile}_{syear}_{eyear}_{self.region}.log"
    ofile=f"transitions_{syear}_{eyear}_{self.region}_{varn}"
    ...
    for inivar in for_1:
        for outvar in for_2:
            ...
            if outvar != outvar_condition:
                ...
                cdo.add(input=f"-selvar,{inivar}_to_{outvar} {ifile} dummy.nc", output=f"{ofile}.nc")
                cdo.chname(input=f"{inivar}_to_{outvar},{varn} {ofile}.nc", output=f"dummy.nc")
    shutil.move(f"{sdir}/{self.region}/dummy.nc", f"{ofile}.nc")
    if remap_com == "invertlat":
